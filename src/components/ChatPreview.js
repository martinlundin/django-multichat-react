...
class ChatPreview extends React.Component {
    ...
    connectToSocket(chatid) {
        if (chatid != null && WebSocketInstance.chatSocket[chatid] == null) {
    ...
    render() {
        return (
            <Link to="/chat" id={this.props.chatid} className="chatPreview padding"
                  onClick={() => this.props.openChat(this.props.chatid)}>
